...
console.log("dfdfdf-"+connection.request.url);
			...
			if (connection.request.url.endsWith('/api/login') && connection.request.method === RequestMethod.Post) {
				...
				// default account
				if (params.email === 'juan@uci.cu' && params.password === 'juan') {
					filteredUsers[0] = {
						fullName: 'Juan',
						email: 'juan@uci.cu',
						password: 'juan',
			...
			if (connection.request.url.endsWith('/api/users') && connection.request.method === RequestMethod.Get) {
				...
				if (connection.request.headers.get('apiKey') === token) {
